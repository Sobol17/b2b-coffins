...
import { ASSIGNEE_ROLES } from '../../src/lib/types/crm-request';
...
const roles = fc.uniqueArray(fc.constantFrom(...ASSIGNEE_ROLES), { maxLength: 3 });
...
describe('the flag of a request without an assignee (C4)', () => {
	...
	it('asks a finished request for a driver, whoever else is on it', () => {
		expect(lacksAssignee('ready', ['carpenter', 'painter'])).toBe(true);
		expect(lacksAssignee('ready', ['driver'])).toBe(false);
	});

	it('never raises once the request left the workshop', () => {
		assertProperty(
			fc.property(
				fc.constantFrom('delivered', 'awaiting_payment', 'paid', 'cancelled', 'rejected' as const),
				roles,
				(status, assigned) => !lacksAssignee(status, assigned)
			)
		);
	});
});
...
describe('the flag of a long wait for payment (C4)', () => {
		...
		assertProperty(
			fc.property(schemes, fc.integer({ min: 0, max: 60 }), (scheme, days) => {
				...
				const flags = attentionFlags(
					{ status: 'awaiting_payment', assigneeRoles: [], deliveredAt, scheme },
					now
				);
	...
	it('never raises for a stock request or outside awaiting payment', () => {
		...
		assertProperty(
			fc.property(fc.constantFrom(...REQUEST_STATUSES), schemes, (status, scheme) => {
				const stock = attentionFlags(
					{ status, assigneeRoles: ['driver'], deliveredAt: long, scheme: null },
					now
				);
				const other = attentionFlags(
					{ status, assigneeRoles: ['driver'], deliveredAt: long, scheme },
					now
				);
				return (
	...
	it('does not raise before the delivery moment is known', () => {
		expect(
			attentionFlags(
				{ status: 'awaiting_payment', assigneeRoles: [], deliveredAt: null, scheme: 'on_fact' },
				now
			)
		).toEqual([]);
	});
