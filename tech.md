...
|---|---|---|
| Поле страницы | `#b5d0ec`: Baby Blue Eyes на ступень светлее, насыщенность 59 % | `--color-surface` → `--color-brand-250` |
...
| Второстепенный текст | `#3f4d63`, 5.4 на поле | `--color-fg-muted` |
...
| Ссылка, активный пункт, акцентная цифра | `#2f4f7f` (5.2 на поле, 8.3 на белом), hover `#1c2b48` | `--color-link`, `--color-link-hover` (новые, v1.29) |
...
Шкалы для редких мест, где семантического токена не хватает: акцент `--color-brand-100…900` и промежуточная `--color-brand-250` под поле (`#e1ebf3`, `#c4d8e5`, `#b5d0ec`, `#a7c7e7`, `#8eb1d1`, `#40679b`, `#2f4f7f`, `#253a5c`, `#1c2b48`, `#121d33`), холодный нейтральный `--color-neutral-100…900` (`#eef4fa`, `#e8ecef`, `#dde3e8`, `#c5ced7`, `#9aa6b4`, `#4d5b70`, `#3f4d63`, `#36435a`, `#1c2b48`), серо-голубой второй акцент `--color-mist-100…900` (`#f0f4f7`, `#e2e9ef`, `#c4d8e5`, `#a9bccb`, `#8b9fb1`, `#6f8aa3`, `#4f6a84`, `#3a5068`, `#26374b`). `brand-500` держит контраст 3.6 на поле и годится только для крупного текста от 24 px (акцентное слово лендинга, номера шагов). Тона статусов `--color-tone-*` перенастраиваются на эти шкалы, словарь тонов в `status.ts` не меняется.
