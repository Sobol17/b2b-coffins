...
src/
  ...
  lib/
    ...
    server/                       только сервер, в браузер не попадает
      ...
      landing/                    landing.service.ts, landing.repository.ts: лендинг гостя (v1.36)
  ...
  routes/
    ...
    (portal)/                     контур контрагента, адреса /portal/*
      ...
      portal/
        ...
        notifications/read/       POST +server.ts: отметка прочтения ленты (v1.35)
    ...
    api/
      ...
      public/works/[id]/+server.ts  обложка опубликованной модели гостю (v1.36)
      health/+server.ts
...
Коды guard-ов: `stockCovered`, `pricesFixed`, `fullyPaid`, `deliveryFilled` (`GUARD_CODES` в `lib/types/request.ts`). Коды эффектов: `audit`, `shipStockItems`, `freezeCharity`, `emit:request.ready`, `emit:request.delivered`, `emit:request.paid` (`EFFECT_CODES`, v1.41).
...
export interface LandingWorkDto { coverMediaId: number; title: string; }
export interface LandingCharityDto { fundTitle: string; fundUrl: string | null; ratePercent: number; }
...
// crm-shop.ts — the shop floor (C5, v1.41). Work goes by position, a request is filled from stock. No money keys.
export const SHOP_REQUEST_LIMIT = 50;               // requests in work shown at once, the number search reaches the rest
export const SHOP_PRODUCE_MAX = 999;                // pieces in one production mark
export interface ShopPositionDto {                   // a stock position of a product: the variant plus its colour
  variantId: number; optionId: number | null;
  productTitle: string; sku: string; sizeCode: string; materialTitle: string; colorTitle: string | null;
}
export interface ShopQueueRowDto extends ShopPositionDto {
  neededQty: number;                                 // pieces the requests in work still lack
  stockQty: number;                                  // balance of the position, never below zero
  requestCount: number; nearestDeliveryAt: string | null; hasUrgent: boolean;
  canProduce: boolean;                               // false when the variant has no stock item
}
export interface ShopLineDto extends ShopPositionDto { itemId: number; qty: number; filledQty: number; }
export interface ShopRequestDto {
  id: number; number: string; priority: RequestPriority; isStockRequest: boolean; counterpartyName: string | null;
  deliveryAt: string | null; unitCount: number; filledCount: number; lines: ShopLineDto[];
  canAssemble: boolean;                              // every line filled: guard stockCovered of tech.md 6.2
}
export interface ShopDto { queue: ShopQueueRowDto[]; requests: ShopRequestDto[]; requestTotal: number; }
...
**P12. Уведомления в приложении и канал МАКС.** Таблица `notification_feed`, запись строки фида в `notification.fanout`, колокольчик в шапке портала сниппетом `bell` у `ContourShell` (кит держит место, разметку и тексты панели даёт портал, как в поиске v1.30), список последних десяти событий со ссылкой на заявку, отметка прочтения при открытии списка через `POST /portal/notifications/read`, полная лента на экране уведомлений. Канал `max` входит в `NOTIFICATION_CHANNELS`, в матрицу и в форму личных настроек, в `LIVE_CHANNELS` не входит. Живого обновления счётчика по SSE нет: персонального топика §7.4 не заводим.
...
**P14. Лендинг: небесный фон.** Компонент `LandingSky` кладёт под лендинг слой `fixed inset-0 -z-10`: градиент поля на CSS, облака на SVG по §18.2. Широкое и узкое поля облаков переключаются на `sm`. Шапка лендинга переходит с заливки на `backdrop-blur`. Портал и CRM слой не получают.
**DoD:** гость на `/` видит небо с облаками и на десктопе, и на телефоне, горизонтального скролла нет, на `/portal` слоя нет.
...
| R2+. Правки бизнеса от 19.09.2026 | P10–P14 |
...
Фон лендинга собран слоем `LandingSky`: поле рисует вертикальный градиент `--color-brand-300` → `--color-brand-150` → `--color-brand-100`, облака рисует SVG белым `--color-surface-raised` под радиальным градиентом с плотностью 0.16, 0.24 и 0.3 по трём планам. Своих цветов слой не заводит и в палитру ничего не добавляет. Вектор взят, чтобы иллюстрация не рассыпалась на ретине; мягкий край даёт радиальный градиент, а не `feGaussianBlur`: фильтр на весь экран дороже, а результат тот же. Полей два, широкое и узкое, они переключаются на `sm`: `preserveAspectRatio="xMidYMid slice"` срезает у телефона две трети широкого поля вместе с облаками. Анимации нет. Портал и CRM слой не получают, под ними остаётся ровная заливка `--color-surface`.
