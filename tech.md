...
|---|---|---|
| **Стадия 0. Каркас** | Слайсы K1–K6 | Приложение запускается локально одной командой, работают миграции, сид, аутентификация, UI-кит, очередь, эталонная вертикаль |
| **Стадия 1. Фичи слайсами** | Этап 1 (P1–P9), Этап 2 (C1–C14) | Рабочая система |
...
| UI-база | shadcn-svelte (bits-ui) | Единственная UI-база на оба контура: CRM целиком и базовые компоненты портала. Примитивы кастомизируются, свои с нуля не пишем |
...
| Web Push | `web-push`, VAPID | Драйвер `PushDriver` за тем же интерфейсом |
...
| Линт | ESLint + Prettier + `svelte-check` | Гейт CI |
| PWA | `manifest.webmanifest` + push-only service worker (`src/service-worker.ts`) | Установка на домашний экран и приём Web Push. Кеширования и офлайн-режима нет, отдельный PWA-плагин не тянем |
...
Примитивы собираются в каркасе (слайс K4) командой `npx shadcn-svelte@latest add <component>` и дальше живут в репозитории как обычный код: правим стили под токены проекта, не форкаем и не переписываем с нуля. Рендерятся в `routes/(dev)/kitchen-sink`. Слайс не пишет свой `<table>` и свою модалку.
...
Порог покрытия: доменный слой `src/lib/domain/**` не ниже 90% строк, гейт CI падает ниже порога. Остальное покрытием не меряем.
...
Только гейт на PR, без деплоя: `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm check` (svelte-check + tsc), `pnpm test:unit`, `pnpm test:e2e`, `pnpm build`, прогон миграций на временном файле SQLite, запуск сида и смоук-запрос `/api/health` на собранной сборке.
...
Ветка `main` защищена: мёрдж только через PR с зелёным гейтом. Работаю один, поэтому self-review по чек-листу §13.3 обязателен, апрув формальный.
...
- эталонная вертикаль в `routes/(portal)/profile` работает и покрыта тестами;
- приложение ставится на домашний экран и принимает тестовый push.
...
**DoD:** каждый примитив отрендерен в `kitchen-sink`, тач-цели не меньше 44 px, приложение ставится на домашний экран.
...
**K6. PWA-оболочка.** `manifest.webmanifest` с иконками и shortcuts, регистрация push-only service worker по §17, обработчики `push` и `notificationclick`, кнопка установки и инструкция для iOS. Кеширование, офлайн-фолбэк и фоновая синхронизация не делаются. Отправка push подключается в P9.
**DoD:** приложение ставится на домашний экран Android и iOS и открывается в standalone-режиме, тестовый push приходит и открывает нужный экран, Cache Storage пуст.
...
**P9. Уведомления портала и стабилизация этапа.** Драйверы email и Web Push на реальных ключах, шаблоны, матрица «событие × роль × канал», личные настройки пользователя, лог доставки и ретраи, подключение push к service worker из K6. Далее: сквозные e2e по портальным ролям, адаптив, a11y, аудит утечки цен и прямых ссылок, инструкция контрагенту.
**DoD:** контрагент получает письмо и push по своим событиям, отправки видны в логе, этап 1 демонстрируется целиком на локальной сборке.
...
**C6. Водительское рабочее место и доставка.** Список заявок в работе для планирования, push при переходе в `ready`, список готовых с адресом, контактом и кнопкой навигации, «Доставлено» с переводом в `delivered` и автопереходом в `awaiting_payment`, отказ при доставке с компенсирующими движениями, просмотр накладной.
**DoD:** водитель получает push в момент готовности и доводит заявку до `delivered`, отказ откатывает складские движения обратными строками.
...
**C12. Уведомления CRM.** Полная матрица событий CRM, шаблоны с переменными, предпросмотр и тестовая отправка, персональные настройки, лог доставки, отзыв протухших push-подписок.
...
| Релиз | Слайсы |
|---|---|
| R1. Приём заявок | K1–K6, P1–P7 |
| R2. Портал целиком | P8, P9 |
| R3. Производство и доставка | C1–C7 |
| R4. Склад, выплаты, отчётность | C8–C14 |
...
Рядом два дешёвых страховочных механизма: `resolve('/crm/requests/[id]', { id })` вместо строковой сборки URL, чтобы переименование роута ломало сборку, а не прод; и `svelte-check` со `strict` в гейте CI, который ловит `data` без нужного поля и необязательные ценовые поля, использованные без проверки.
...
Подписка создаётся в слайсе P9 на реальных VAPID-ключах, хранится в `push_subscriptions`, протухшие подписки отзываются джобом `session.cleanup`.
...
- ручная проверка установки и получения push на Android Chrome и iOS Safari в DoD слайсов K6, P9, C5, C6.
