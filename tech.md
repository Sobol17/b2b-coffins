...
| v1.29 | Палитра «Ангел» переведена на пять цветов заказчика: midnight blue `#1c2b48` (акцент и основной текст), Platinum `#e8ecef` (поле страницы), light blue grey `#c4d8e5`, Baby Blue Eyes `#a7c7e7` и Cool Cerulean `#8eb1d1` (только заливки: как текст на белом они дают контраст 1.5–2.2). Акцент и текст совпали по цвету, поэтому добавлены токены `--color-link` и `--color-link-hover`: ими красятся ссылки, активный пункт навигации, ghost-кнопка и акцентные цифры, `text-brand` остался только на светлых заливках. Тона статусов идут по шкале: заявка светлее, в работе средний, готов к выдаче насыщенный. Цвета манифеста §17 взяты из новой палитры. Юнит `visual-tokens` проверяет контраст текстовых пар по WCAG AA. §17 и §18.2 приведены к этому |
...
|---|---|---|
| Поле страницы | `#e8ecef` Platinum | `--color-surface` |
| Панель, карточка | `#ffffff` | `--color-surface-raised` |
| Врезка, поле ввода, степпер | `#f3f5f7` | `--color-surface-muted` |
| Серый чип, кнопка в карточке | `#e8ecef`, hover `#c4d8e5` | `--color-chip`, `--color-chip-hover` |
| Основной текст | `#1c2b48` midnight | `--color-fg` |
| Второстепенный текст | `#4d5b70` | `--color-fg-muted` |
| Подпись, плейсхолдер, артикул | `#66758a` | `--color-fg-faint` |
| Разделитель | `#1c2b48` на 10 % | `--color-border` |
| Контур будущего шага | `#8eb1d1` | `--color-border-strong` |
| Акцент (заливка) | `#1c2b48`, hover `#2c4468`, active `#121d33` | `--color-brand`, `--color-brand-hover`, `--color-brand-active` |
| Ссылка, активный пункт, акцентная цифра | `#3d5f8f`, hover `#1c2b48` | `--color-link`, `--color-link-hover` (новые, v1.29) |
| Мягкий акцент (баннер, тег «Заявка», тост) | фон `#e1ebf3`, текст `#1c2b48` | `--color-tone-info-soft`, `--color-tone-info` |
...
| Выделение текста, фон аватара | `#a7c7e7`, `#c4d8e5` | `--color-brand-300`, `--color-brand-200` |
...
Шкалы для редких мест, где семантического токена не хватает: акцент `--color-brand-100…900` (`#e1ebf3`, `#c4d8e5`, `#a7c7e7`, `#8eb1d1`, `#5b82ad`, `#3d5f8f`, `#2c4468`, `#1c2b48`, `#121d33`), холодный нейтральный `--color-neutral-100…900` (`#f3f5f7`, `#e8ecef`, `#dde3e8`, `#c5ced7`, `#9aa6b4`, `#66758a`, `#4d5b70`, `#36435a`, `#1c2b48`), серо-голубой второй акцент `--color-mist-100…900` (`#f0f4f7`, `#e2e9ef`, `#c4d8e5`, `#a9bccb`, `#8b9fb1`, `#6f8aa3`, `#4f6a84`, `#3a5068`, `#26374b`). `brand-500` держит контраст 3.4 на поле и годится только для крупного текста от 24 px (акцентное слово лендинга, номера шагов). Тона статусов `--color-tone-*` перенастраиваются на эти шкалы, словарь тонов в `status.ts` не меняется.
