...
| `MoneyInput` | `bind:valueMinor`, `placeholder` (обязателен), ввод в целых рублях, хранение в копейках (v1.32) |
...
- Интерфейс и XLSX показывают суммы в целых рублях без копеек: `formatMinor` округляет при выводе, в данных копейки остаются (v1.32).
...
| Контур будущего шага | `#8eb1d1` Cool Cerulean | `--color-border-strong` |
...
| Тег «Готов к выдаче» | фон `#b9dcae`, текст `#1f3d1b`, контраст 8.0 (v1.31) | `--color-tone-ready-soft`, `--color-tone-ready` |
